...
public class DeedDisplayPanelController extends AbstractNavigablePanelController
{
  ...
  {
    return Labels.getLabel("deed.display.title",new Object[]{_deed.getName()});
  }
  ...
  private JPanel build()
  {
    ...
    TitledBorder rewardsBorder=GuiFactory.buildTitledBorder(Labels.getLabel("deed.display.rewards"));
    ...
    detailsPane.setBorder(GuiFactory.buildTitledBorder(Labels.getLabel("deed.display.details")));
  ...
  private JPanel buildPrivateTop()
  {
    ...
    // Line 2
    {
      ...
      c.gridy++;
      // Category
      panelLine.add(GuiFactory.buildLabel(Labels.getLabel("deed.display.category")+": "));
      _category=GuiFactory.buildLabel("");
      panelLine.add(_category);
      // Type
      panelLine.add(GuiFactory.buildLabel(Labels.getLabel("deed.display.type")+": "));
    ...
    // Line 3 (challenge level)
    {
      ...
      c.gridy++;
      // Challenge level
      panelLine.add(GuiFactory.buildLabel(Labels.getLabel("deed.display.level")+": "));
    ...
    // Line 4 (requirements)
    {
      ...
      c.gridy++;
      // Requirements
      panelLine.add(GuiFactory.buildLabel(Labels.getLabel("deed.display.requirements")+": "));
    ...
    // Line 6 (quest pack)
    {
      ...
      c.gridy++;
      // Quest pack
      panelLine.add(GuiFactory.buildLabel(Labels.getLabel("deed.display.contentsPack")+": "));
    ...
    if (requirement!=null)
    {
      ...
      achievablesRequirementsPanel.setBorder(GuiFactory.buildTitledBorder(Labels.getLabel("deed.display.questsDeedsRequirements")));
      c.gridy++;
    }
    ...
    if (worldEventsConditions!=null)
    {
      ...
      if (expression!=null)
      {
        ...
        worldEventConditionsPanel.setBorder(GuiFactory.buildTitledBorder(Labels.getLabel("deed.display.context")));
        c.gridy++;
      }
    }
    return panel;
  ...
  private JButton buildMapsButton(WindowController parent)
  {
    ...
    if (hasGeoData)
    {
      ...
      toggleMap=GuiFactory.buildButton(Labels.getLabel("deed.display.map"));
  ...
  private String buildHtml()
  {
    ...
    sb.append("<html><body>");
    sb.append(Labels.getLabel("deed.display.descriptionTitle"));
  ...
  private String buildAttributesString()
  {
    ...
    if (isMonsterPlay)
    {
      ...
      sb.append(Labels.getLabel("deed.display.monsterPlay"));
    }
    // Hidden
    ...
    {
      if (sb.length()>0) sb.append(", ");
      sb.append(Labels.getLabel("deed.display.hidden"));
    }
